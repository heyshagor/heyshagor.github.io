...
//    try {
//       const res = await fetch(`${PROXY}/v1/blocks/${id}/children?page_size=100`, {
//          headers: {
//             'Content-Type': 'application/json'
//          }
//       });
...
//             html += `<li>${rt(blocks[i].bulleted_list_item.rich_text)}</li>`;
//             i++;
//          }
//          html += '</ul>';
//          continue;
...
//             html += `<li>${rt(blocks[i].numbered_list_item.rich_text)}</li>`;
//             i++;
//          }
//          html += '</ol>';
//          continue;
...
//    switch (b.type) {
//       case 'paragraph':
//          return `<p class="n-p">${rt(b.paragraph.rich_text)||'<br>'}</p>`;
//       case 'heading_1':
//          return `<h2 class="n-h1">${rt(b.heading_1.rich_text)}</h2>`;
//       case 'heading_2':
//          return `<h2 class="n-h2">${rt(b.heading_2.rich_text)}</h2>`;
//       case 'heading_3':
//          return `<h3 class="n-h3">${rt(b.heading_3.rich_text)}</h3>`;
//       case 'code': {
...
//       }
//       case 'quote':
//          return `<div class="n-quote"><p>${rt(b.quote.rich_text)}</p></div>`;
//       case 'callout': {
...
//       }
//       case 'toggle':
//          return `<details class="n-toggle"><summary>${rt(b.toggle.rich_text)}</summary><div class="n-toggle-body"><p class="n-p" style="font-size:13px;color:var(--text3)">Toggle content loads in Notion.</p></div></details>`;
//       case 'to_do':
//          return `<div class="n-todo"><div class="n-check ${b.to_do.checked?'done':''}">${b.to_do.checked?'✓':''}</div><div class="n-todo-text ${b.to_do.checked?'done':''}">${rt(b.to_do.rich_text)}</div></div>`;
//       case 'bookmark': {
...
//       }
//       case 'equation':
//          return `<div class="n-equation">${esc(b.equation.expression)}</div>`;
//       case 'table_of_contents':
//          return `<div class="n-callout cb-blue"><div class="n-callout-icon">📋</div><div class="n-callout-text" style="font-family:'JetBrains Mono',monospace;font-size:11px">table of contents</div></div>`;
//       default:
//          return '';
//    }
// }
...
function renderBlocks(blocks) {
  ...
  while (i < blocks.length) {
    ...
    // TABLE — open table and store its id
    if (b.type === 'table') {
      currentTableId = b.id;
      const hasHeader = b.table?.has_column_header || false;
      html += `<div class="n-table-wrap"><table class="n-table" id="tbl-${b.id}" data-header="${hasHeader}"><tbody></tbody></table></div>`;
      i++;
      continue;
    }

    // TABLE ROW — store as placeholder to inject after HTML is parsed
    if (b.type === 'table_row') {
      const tblId = currentTableId ? `tbl-${currentTableId}` : null;
      if (tblId) {
        const cellsHtml = (b.table_row?.cells || []).map(cell => `<td>${rt(cell)}</td>`).join('');
        html += `<tr data-tbl="${tblId}">${cellsHtml}</tr-placeholder>`;
      }
      i++;
      continue;
    }
  ...
  // Inject table rows into their tables using DOM manipulation
  const wrapper = document.createElement('div');
  wrapper.innerHTML = html;

  wrapper.querySelectorAll('tr[data-tbl]').forEach(row => {
    const tblId = row.getAttribute('data-tbl');
    const tbl = wrapper.querySelector(`#${tblId}`);
    const tbody = tbl ? tbl.querySelector('tbody') : null;
    if (tbody) {
      const isHeader = tbl.getAttribute('data-header') === 'true' && tbody.rows.length === 0;
      if (isHeader) {
...
function block(b) {
  switch (b.type) {
    ...
    case 'code': {
      const lang = b.code.language || 'code';
      const code = esc(b.code.rich_text.map(r => r.plain_text).join(''));
      const cap = b.code.caption?.length
        ? `<div style="font-family:'JetBrains Mono',monospace;font-size:11px;color:var(--text3);padding:8px 16px;border-top:1px solid var(--border)">${rt(b.code.caption)}</div>`
        : '';
      return `<div class="n-code"><div class="n-code-bar"><span>${lang}</span><button class="copy-btn" onclick="copyCode(this)">copy</button></div><pre><code>${code}</code></pre>${cap}</div>`;
    }
    case 'quote':
    ...
    case 'callout': {
      const icon = b.callout.icon?.emoji || '💡';
      const c = b.callout.color || '';
      const cls = c.includes('blue') ? 'cb-blue'
        : c.includes('yellow') ? 'cb-yellow'
        : c.includes('red') ? 'cb-red'
        : c.includes('green') ? 'cb-green'
        : c.includes('purple') ? 'cb-purple' : '';
      return `<div class="n-callout ${cls}"><div class="n-callout-icon">${icon}</div><div class="n-callout-text">${rt(b.callout.rich_text)}</div></div>`;
    }
    case 'divider':
      return `<hr class="n-divider"/>`;

    // IMAGE — constrained, never overflows
    case 'image': {
      const url = b.image.type === 'external' ? b.image.external.url : b.image.file?.url || '';
      const cap = b.image.caption?.length ? rt(b.image.caption) : '';
      if (!url) return '';
      return `<figure class="n-image">
        <img src="${url}" alt="${cap || 'image'}" loading="lazy"
          style="max-width:100%;max-height:480px;width:auto;height:auto;margin:0 auto;display:block;object-fit:contain;"/>
    ...
    case 'equation':
      return `<div class="n-equation">${esc(b.equation.expression)}</div>`;
    case 'table_of_contents':
      return `<div class="n-toc" id="tocBlock"><div class="n-toc-title">📋 table of contents</div><div id="tocList"></div></div>`;
    default:
      return '';
  }
}
...
function buildTOC() {
  ...
  if (!toc) return;
  const headings = document.querySelectorAll('#postContent .n-h1, #postContent .n-h2, #postContent .n-h3');
  if (!headings.length) {
  ...
  headings.forEach((h, i) => {
    const id = 'heading-' + i;
    h.id = id;
    const level = h.classList.contains('n-h1') ? 0 : h.classList.contains('n-h2') ? 1 : 2;
...
function rt(arr) {
  ...
  return arr.map(t => {
    ...
    if (a.underline) s = `<span class="rt-under">${s}</span>`;
    if (a.color && a.color !== 'default') {
      const cls = a.color.includes('_background')
        ? 'bg-' + a.color.replace('_background', '')
        : 'c-' + a.color.replace(/_background/, '');
      s = `<span class="${cls}">${s}</span>`;
    }
    if (t.href) s = `<a href="${t.href}" class="rt-link" target="_blank">${s}</a>`;
    return s;
...
// FIXED: proper HTML entity escaping (your original had broken entities)
function esc(s) {
  return String(s || '')
    .replace(/&/g, '&amp;')
    .replace(/</g, '&lt;')
